...
const PropertyRowMoreButton = (props: PropertyRowMoreButtonProps) => {
    ...
    const toggleExpandedPropertyId = () => {
        setExpandedPropertyId(isExpanded ? null : props.propertyId)
    }

    return (
        <>
            {isExpanded && (
                <Frame height={26} width={89} stroke='#D9D9D9' fill="#fff" positioning='absolute' x={252} y={0} cornerRadius={{
                    topLeft: 4,
                    topRight: 0,
                    bottomLeft: 4,
                    bottomRight: 0,
